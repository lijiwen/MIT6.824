...
func doMap(
	...
	if err != nil{
		log.Fatalf("common_map.go doMap read file %s error\n", inFile)
	}
	...
	for _, kvPair := range kvPairs{
		n = ihash(kvPair.Key) % nReduce
		if res[n] == nil{
			res[n] = make([]KeyValue, 0)
		}
		res[n] = append(res[n], kvPair)
	}

	for i, iRes := range res{
		b, err := json.Marshal(iRes)
		if err != nil{
			log.Fatalf("common_map.go %d json marshal error\n", i)
		}

		outputFileName := reduceName(jobName, mapTaskNumber, i)
		err = ioutil.WriteFile(outputFileName, b, 0666)
		if err != nil{
			log.Printf("common_map.go %d write file error\n", i)
			log.Fatal(err)
		}
	}
}
